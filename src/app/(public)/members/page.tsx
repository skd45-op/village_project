...
export default async function MembersPage() {
  ...
  const full = isMemberOrAbove(viewer); // member+ sees contact details

  // Contact fields are only rendered for member+ viewers. In RSC, fields that
  // aren't rendered are never serialized to the client, so guests don't receive them.
    ...
    <div>
      ...
      {!full && (
        <p className="mb-4 text-sm text-neutral-500">
          You&apos;re viewing the public directory (name &amp; photo only). Members can see contact details.
        </p>
      )}
          ...
          <Card key={m.id}>
            <CardBody className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                {m.role !== "member" && <Badge tone={m.role === "superadmin" ? "red" : "blue"}>{m.role}</Badge>}
                {full && (
